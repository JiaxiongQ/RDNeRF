...
# from ddp_model_o import NerfNetWithAutoExpo
import time
from data_loader_split import load_data_split
import numpy as np
from tensorboardX import SummaryWriter
from utils import img2mse, mse2psnr, img_HWC2CHW, colorize, TINY_NUMBER, get_gradient, get_smooth
...
def intersect_sphere2(rays_o, rays_d):
...
def render_single_image(rank, world_size, models, ray_sampler, chunk_size):
    ...
    for s in range(len(ray_batch_split['ray_d'])):
        ...
        for m in range(models['cascade_level']):
            ...
            if m == 0:
                fg_far_depth = 10.0*torch.ones_like(min_depth)
                fg_near_depth = min_depth  # [..., ]
                t_vals = torch.linspace(0., 1., steps=N_samples)
                fg_depth = t_vals.expand([N_rays, N_samples])                 
                ...
                fg_depth = ret['fg_z_vals'].clone().detach()
            ...
            with torch.no_grad():
                ret = net(ray_o, ray_d,min_depth,fg_far_depth, fg_depth,idx,False)
   
            for key in ret:
                if key not in ['fg_weights']:
...
def log_view_to_tb(writer, global_step, log_data, gt_img, mask, prefix=''):
    ...
    for m in range(len(log_data)):
        ...
        depth1 = log_data[m]['fg_far_depth']
        depth_im1 = img_HWC2CHW(colorize(depth1, cmap_name='jet', append_cbar=True,
                                        mask=mask))
        writer.add_image(prefix + 'level_{}/rel_depth'.format(m), depth_im1, global_step)

        depth = log_data[m]['fg_far_depth2']
        depth_im = img_HWC2CHW(colorize(depth, cmap_name='jet', append_cbar=True,
                                        mask=mask))
        writer.add_image(prefix + 'level_{}/far_depth'.format(m), depth_im, global_step)
...
def create_nerf(rank, args):
    ...
    for m in range(models['cascade_level']):
        ...
        net = NerfNetWithAutoExpo(args, optim_autoexpo=args.optim_autoexpo, img_names=img_names,m=m).to(rank)
        ...
        optim = torch.optim.Adam(net.parameters(), lr=args.lrate, betas=(0.9, 0.999))
        scheduler = MultiStepLR(optim, milestones=[150000,300000], gamma=0.5)
        models['net_{}'.format(m)] = net
        models['optim_{}'.format(m)] = optim
        models['sche_{}'.format(m)] = scheduler
    start = -1
...
def ddp_train_nerf(rank, args):
    ...
    ray_samplers = load_data_split(args.datadir, args.scene, split='train',
                                   try_load_min_depth=args.load_min_depth, skip=1, test_flag=False)
    val_ray_samplers = load_data_split(args.datadir, args.scene, split='validation',
                                       try_load_min_depth=args.load_min_depth, skip=1, test_flag=True)
    ...
    for global_step in range(start+1, start+1+args.N_iters):
        ...
        ray_batch = ray_samplers[i].random_sample(args.N_rand, center_crop=False)#scannet True
        ...
        fg_far_depth = 10.0*torch.ones_like(ray_batch['min_depth'])
        for m in range(models['cascade_level']):
            optim = models['optim_{}'.format(m)]
            scheduler = models['sche_{}'.format(m)]
            net = models['net_{}'.format(m)]
            # sample depths
            N_samples = models['cascade_samples'][m] 
            ...
            optim.zero_grad()
            ret = net(ray_batch['ray_o'], ray_batch['ray_d'], ray_batch['min_depth'], fg_far_depth, fg_depth, m, True, img_name=ray_batch['img_name'])
            all_rets.append(ret)
            ...
            depthr_gt = ray_batch['depthr'].to(rank)
            depthr = ret['fg_far_depth']

            # depth2mae = nn.L1Loss()
            # mask = (depthr_gt>0.001).detach_()
            # dr_loss = img2mse(depthr,depthr_gt)
            # dr_loss = F.smooth_l1_loss(depthr, depthr_gt, size_average=True)
            dr_loss = F.mse_loss(depthr, depthr_gt, reduction='mean')

            # #alpha loss
            # _alpha = ret['alpha']
            # alpha_loss = torch.log1p(1. / 0.11 * (_alpha+TINY_NUMBER).float() * (1 - _alpha.float() + TINY_NUMBER)).mean().type_as(_alpha)
            for param_group in optim.param_groups:
                lr = param_group['lr']
 
            loss = loss #+ 0.1*dr_loss #+ alpha_loss 
                
            scalars_to_log['level_{}/lr'.format(m)] = lr
            scalars_to_log['level_{}/loss'.format(m)] = loss.item()
            scalars_to_log['level_{}/rgb_loss'.format(m)] = rgb_loss.item()
            scalars_to_log['level_{}/depthr_loss'.format(m)] = dr_loss.item()
