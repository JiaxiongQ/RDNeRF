...
def load_data_split(basedir, scene, split, skip=1, try_load_min_depth=True, only_img_files=False, test_flag=False):
    ...
    base_dir = '{}/{}'.format(basedir, scene)

    if only_img_files:
        img_files = find_files('{}/color'.format(split_dir), exts=['*.png', '*.jpg'])
        return img_files
    ...
    train_imgfile = find_files('{}/{}/train/color'.format(basedir, scene), exts=['*.png', '*.jpg'])[0]
    ...
    for i in range(cam_cnt):
        ...
        # read max depth
        try:
            max_depth = float(open('{}/max_depth.txt'.format(base_dir)).readline().strip())
        except:
            max_depth = None
        ray_samplers.append(RaySamplerSingleImage(H=H, W=W, intrinsics=intrinsics, c2w=pose,
                                                  ...
                                                  max_depth=max_depth, test_flag=test_flag))
